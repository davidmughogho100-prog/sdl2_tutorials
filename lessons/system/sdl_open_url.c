...
int main()
{
	...
	SDL_Window *win = SDL_CreateWindow("sdl url operations", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_WIDTH,SCREEN_HEIGHT,0);
	...
	char *urls = "1 ==> example.com\n"
				 "2 ==> python.org\n"
				 "3 ==> song.mp3\n";

	SDL_Surface *text_surface = TTF_RenderText(arial_font, urls, text_color, text_color_bg);
	...
	while (running)
	{
		while (SDL_PollEvent(&ev))
		{
			...
			if (ev.type == SDL_KEYDOWN)
			{
				...
				{
					SDL_OpenURL("https://example.org/");
				}

				if (ev.key.keysym.sym == SDLK_2)
				{
					SDL_OpenURL("https://python.org/");
				}

				if (ev.key.keysym.sym == SDLK_3)
				{
					// open local files
					SDL_OpenURL("./assets/song.mp3");
				}
			}
		}
